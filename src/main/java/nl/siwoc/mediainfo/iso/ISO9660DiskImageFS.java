...
public class ISO9660DiskImageFS {
	...
	private void parseFile(byte[] data) {
		...
		String flags = String.format(
				"%8s",
				new Object[] { Integer.toBinaryString(ISOUtils.UValue(data[25])) }).replace(' ', '0');
		boolean dir = false;
		if (flags.charAt(6) == '1')
			dir = true;
		int length = ISOUtils.UValue(data[32]);
		...
		int ss = ISOUtils.readLittleEndianWord(Arrays.copyOfRange(data, 2, 6));
		int sz = ISOUtils.readLittleEndianWord(Arrays
	...
	public byte[] getFile(String filePath) {
		if ((this.files != null) && (this.files.containsKey(filePath))
			...
			try {
				...
				this.raf.seek(this.files.get(filePath).startSector * SECTORSIZE);
